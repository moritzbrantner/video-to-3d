...
- **Surface-first model output — current slice.** Treat the mesh as the product output rather than letting the diagnostic point clouds visually dominate. Geometry controls viewer framing instead of the camera path. Accepted surface triangles render as an opaque shaded model by default; sparse points, dense samples, and camera markers move to an explicit evidence view. The Rust mesh can bridge exactly one missing grid sample only when the bridge uses existing accepted fused vertices and passes a stricter 8% relative-depth continuity gate, so isolated evidence holes do not unnecessarily fragment otherwise coherent surfaces.
