...
- **Memory-aware native and WASM execution — next.** Add progressive/chunked processing where full-resolution depth or texture working sets would exceed practical browser memory budgets, without changing the existing acceptance gates.
...
Next implementation slice: make dense reconstruction and texture working sets memory-aware and progressive in native and WASM execution while preserving exact geometry/provenance acceptance. After that, add the owned native-provider interchange form before integrating a learned backend.
