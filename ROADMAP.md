...
- **Trevi Pages field canary — planned with the Pages demo.** Keep an openly licensed Trevi Fountain clip available directly in GitHub Pages as a one-click real-video smoke test. Treat it as a difficult field canary with camera motion, water, and changing local appearance, not as ground-truth quality evidence; synthetic fixtures and reference datasets remain authoritative for regression metrics.
