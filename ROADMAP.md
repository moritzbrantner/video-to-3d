...
- **Multi-reference surface coverage and seam fusion — integrated.** Run bounded dense/mesh reconstruction from up to three well-supported registered reference views in the shared reconstruction frame. Cross-reference fusion consumes validated `SurfaceEvidenceRegion` camera-support groups rather than provider-specific patch metadata, aligns only mutually nearest compatible accepted vertices, validates the complete proposed move set against the original accepted topology, and never creates vertices, triangles, or unsupported hole filling.
- **Reference-view texture projection — integrated.** Preserve each accepted dense sample's Rust-owned reference-grid site through the core/WASM/browser contract. A surface triangle is texture-eligible only when all three vertices retain the same unambiguous accepted reference. The browser projects that sampled reference image onto already accepted topology; mixed-reference, ambiguous, missing, or not-yet-loaded evidence falls back to vertex-color shading and never creates, repairs, or promotes geometry. Interactive orbit/zoom temporarily uses the cheap shaded path so texture projection does not dominate pointer latency.
- **Fail-closed dense working-set budget — integrated.** Rust estimates the peak frame, retry/remapping, luminance, multi-reference sample, topology, and packed-WASM output working set before dense reconstruction. A configurable budget suppresses only dense geometry when the estimate is too large, exposes the estimate and budget as browser diagnostics, and leaves sparse reconstruction unchanged. Inputs below budget retain the existing dense acceptance path exactly.
- **Progressive native and WASM dense execution — next.** Chunk reference processing and texture projection within the Rust-owned budget so oversized inputs can make bounded progress without changing geometry, provenance, or acceptance gates.
...
Current boundary: Rust owns camera geometry, reciprocal-consistent depth acceptance, spatial fusion, bounded completion, grid identity, mesh topology, cross-reference seam validation, and provider-neutral evidence validation. `ReconstructionEvidenceView` remains the common provenance-aware boundary for built-in and future learned/native providers. Browser texture projection is appearance-only over already accepted triangles and fails closed to shaded geometry when reference ownership is ambiguous or unavailable. Unsupported gaps remain open. Watertight completion, metric scale, full-resolution depth maps, provider-native execution, unbounded cross-provider surface aggregation, and texture-driven geometry changes remain outside the implemented boundary.

Next implementation slice: process dense references progressively within the Rust-owned working-set budget while preserving exact geometry/provenance acceptance. After that, add the owned native-provider interchange form before integrating a learned backend.
