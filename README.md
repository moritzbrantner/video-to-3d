...
15. Selected non-adjacent keyframe pairs are screened with mutual descriptor matches. Strong candidates are exposed as revisit evidence but do not change geometry by themselves.
16. For an unregistered selected keyframe, a strong direct match back to the calibrated seed frame can map accepted seed features directly to seed 3D landmarks. At least eight such 3D↔2D correspondences are required before the existing robust PnP solver is retried; a failed retry leaves the frame unregistered.
17. Recovered cameras enter exactly the same registered-camera set as ordinary PnP results and therefore participate in subsequent triangulation and bundle adjustment without a second geometry authority.
18. Non-seed feature tracks observed by at least one accepted additional camera are triangulated across registered views. New landmarks require positive depth, sufficient observation support, bounded reprojection error, and a minimum triangulation angle before they are added to the sparse cloud.
19. Bundle adjustment gathers only registered observations that already satisfy the positive-depth and 4 px support boundary, then alternates bounded Huber-weighted Gauss–Newton landmark and camera-pose updates. The two calibrated seed cameras remain fixed to preserve the monocular gauge.
20. Adjusted geometry replaces the pre-adjustment reconstruction only when robust cost improves, reprojection RMSE does not regress, and median reprojection error stays within the explicit no-regression boundary. Otherwise the original accepted PnP/triangulated geometry is retained.

Scale remains arbitrary because monocular video has no metric baseline. Additional registered cameras and sparse landmarks share the seed pair's coordinate frame, and accepted bundle adjustment can refine their joint reprojection consistency without moving that seed gauge. Revisit recovery is deliberately not pose-graph loop closure: it supplies another absolute PnP attempt from already accepted seed geometry but does not redistribute accumulated trajectory drift. The remaining Slice 3 work is to turn validated non-adjacent revisit evidence into bounded global drift correction without weakening the fixed-gauge and fail-closed quality boundaries. Each selected video also remains independent; cross-video reconstruction belongs to slice 6.
