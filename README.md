...
19. Non-seed feature tracks observed by at least one accepted additional camera are triangulated across registered views. New landmarks require positive depth, sufficient observation support, bounded reprojection error, and a minimum triangulation angle before they are added to the sparse cloud.
20. Bundle adjustment gathers only registered observations that already satisfy the positive-depth and 4 px support boundary, then alternates bounded Huber-weighted Gauss–Newton landmark and camera-pose updates. The two calibrated seed cameras remain fixed to preserve the monocular gauge.
21. Adjusted geometry replaces the pre-adjustment reconstruction only when robust cost improves, reprojection RMSE does not regress, and median reprojection error stays within the explicit no-regression boundary. Otherwise the original accepted PnP/triangulated geometry is retained.
